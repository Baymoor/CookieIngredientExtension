...
const EXACT_MATCH_MAP = new Map<string, { category: string, vendor: string, description: string, retention: string }>();
const PATTERN_MATCH_LIST: { regex: RegExp, category: string, vendor: string, description: string, retention: string }[] = [];
...
(function initializeCookieDatabase() {
  ...
  console.log(`Database loaded: ${EXACT_MATCH_MAP.size} exact matches, ${PATTERN_MATCH_LIST.length} patterns.`);
})();

// --- Logic ---

function identifyCookie(cookie: chrome.cookies.Cookie, currentHostname: string): { category: string; vendor: string; description: string; retention: string } {
  ...
  if (!cleanCurrentHost.includes(cleanCookieDomain) && !cleanCookieDomain.includes(cleanCurrentHost)) {
    return {
      category: "Targeting",
  ...
  if (cookieNameLower.includes('sess') || cookieNameLower.includes('csrf') || cookieNameLower.includes('xsrf') || cookieNameLower.includes('token') || cookieNameLower.includes('auth')) {
    return {
      category: "Strictly Necessary",
      vendor: "Heuristic Match",
      description: "The name suggests this is a security token or session ID, essential for keeping you logged in.",
      retention: isSession ? "Session" : "Persistent"
    };
  }

  if (cookieNameLower.startsWith('_g') || cookieNameLower.includes('pixel') || cookieNameLower.includes('ads') || cookieNameLower.includes('tracker')) {
    return {
      category: "Performance",
      vendor: "Heuristic Match",
      description: "The name contains common tracking terms. It is likely measuring your interactions with the page.",
      retention: "Variable"
    };
  }

  if (cookieNameLower.includes('pref') || cookieNameLower.includes('lang') || cookieNameLower.includes('theme') || cookieNameLower.includes('mode')) {
    return {
      category: "Functional",
      vendor: "Heuristic Match",
      description: "The name implies it's remembering a choice you made, like language or dark mode.",
  ...
  if (isSession) {
    return {
      category: "Strictly Necessary",
      vendor: "Unknown",
      description: "This is a temporary session cookie. It usually holds your 'state' while you browse and is deleted when you close the browser.",
      retention: "Session"
    };
  }
  ...
  if (lifespan < oneDay) {
    return {
      category: "Performance",
      vendor: "Unknown",
      description: "This cookie expires very quickly (under 24h). Short-lived cookies are often used for brief analytics or checking if your browser supports cookies.",
  ...
  if (lifespan > oneYear) {
    return {
      category: "Targeting",
      vendor: "Unknown",
      description: "This cookie lasts over a year. Long lifespans are a hallmark of tracking cookies building a long-term profile of you.",
      retention: "Long-term"
    };
  }
  ...
  return {
    category: "Functional",
    vendor: "Unknown",
    description: "This persistent cookie doesn't match known tracking patterns. It's likely remembering your preferences or site state.",
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  showIngredientsBtn?.addEventListener('click', () => {
    ...
    if (isHidden) {
        ...
        setTimeout(() => listContainer?.scrollIntoView({ behavior: 'smooth' }), 100);
    } else {
  ...
  if (scanningView && dashboardView) {
    ...
    try {
      ...
      if (tab?.url && (tab.url.startsWith('http://') || tab.url.startsWith('https://'))) {
        ...
        const storage = await chrome.storage.local.get(`protect_${currentHostname}`);
        ...
        cookies.forEach((cookie) => {
          ...
          else if (info.category === 'Strictly Necessary') badgeClass = 'badge-strict';
        ...
        if (countFunctionalEl) countFunctionalEl.textContent = String(counts.func);
        if (countPerformanceEl) countPerformanceEl.textContent = String(counts.perf);
        if (countTargetingEl) countTargetingEl.textContent = String(counts.target);
        if (countStrictlyNecessaryEl) countStrictlyNecessaryEl.textContent = String(counts.strict);
  ...
  clearBtn?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (tab?.url) {
        ...
        if (countStrictlyNecessaryEl) countStrictlyNecessaryEl.textContent = "0";
        if (cookieListContainer) cookieListContainer.innerHTML = '';
  ...
  protectBtn?.addEventListener('click', async () => {
    ...
    if (currentHostname) {
        if (isProtected) {
            await chrome.storage.local.set({ [`protect_${currentHostname}`]: true });
        } else {
